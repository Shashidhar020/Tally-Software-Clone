...
import { setBackGroundColor } from "../../redux/dashboardSlice";

function Sales() {
    const navigate = useNavigate();
    const{backgroundColor} = useSelector(state=>state.dashboard)
...
useEffect(() => {
 ...
 try {
    ...
    const allSales =  [...allSalesdata].sort((a,b)=>new Date(a.date) - new Date(b.date))
...
useEffect(()=>{
 ...
 const save = async () => {
  if (!account.trim()) {
   ...
   if(account.trim() === 'new'){
    alert("Account Name should not be a new ");
    return;
   }
  const newSale = {
    account,
  ...
  try {
    ...
    // --- Handle Edit Mode ---
    if (ledgerNameParam && ledgerNameParam !== "new") {
      if (isExisting && account !== ledgerNameParam) {
      ...
      if (res.data.success) {
        alert(res.data.success && "Sale updated successfully!");
        console.log("Matched Count:", res.data.matchedCount);
        navigate("/sales");
      } else {
        alert(res.data.message || "Failed to update sale.");
      }

    } else {
      // --- Handle New Sale Creation ---
      if (isExisting) {
        alert("A sale with this account already exists. Please choose a different account or edit it.");
        navigate(`/sales/${encodeURIComponent(account)}`);
        return;
      }
    ...
    alert("An error occurred while saving the sale. Check console for details.");
  }
};
...
{      navigate(`/sales/${encodeURIComponent(ledgerNameParam)}`)
  ...
  const addElement = (item, index) => {
    return (
      <p 
      key={index}  > 
        <span><input type="text" className="input itemNo" style={{marginRight:"50px",width:"50px", border:"none"}} value={item.itemNumber} onChange={(e) => handleItemChange(index, 'itemNumber', e.target.value)}/></span>
        <span ><input type="text" value={item.name} className="input name"
  ...
  <input
    ...
    style={{ marginLeft: "200px",width: "100px", border: "0px transparent" }}
  />
</span>
        <span style={{marginLeft:"10px"}}><input type="text" className="input rate" value={`${item.rate}₹`}    onChange={(e) =>
          {
             const onlyNumber = e.target.value.replace(/\D/g, ""); 
           handleItemChange(index, 'rate', onlyNumber)}} style={{width:"100px",border:"transparent"}}/></span>
        <span
        onKeyUp={(index)=>addNewItem(index)}
        style={{marginLeft:"10px"}}><input type="text" className="input amount" value={`${item.amount} ₹`} onChange={(e) =>
...
const handleSelect = (item) => {
  ...
  return (
    <div style={{width:"100%", display: "flex", flexDirection: "column"}} className="sales-page">
      <div style={{display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px"}}>
        ...
        <div style={{display: "flex", flexDirection: "column", alignItems: "flex-end"}}>
          ...
          <input type="date" onChange={handleChange} value={date} style={{marginBottom: "10px",marginTop:"10px",width: "120px"}} required />
      ...
      <div>
        ...
        <div>
          <p className="p-head" style={{ borderBottom: "2px solid #615f5fff"}}>
            <span  className="p-itemNO" style={{marginRight:"55px"}}>Item No</span>
            <span className="p-name">Name of Item</span>
            <span className="p-quantity" style={{marginLeft: "27%" }}>Quantity</span>
            <span  className="p-rate" style={{marginLeft:"57px"}}>Rate/pcs</span>
            <span className="p-amount" style={{marginLeft:"70px"}}>Amount</span>
          </p>
        </div>
        <div id="list" >
        ...
        <div>
          ...
          <p style={{position:"fixed",top:"93%",width:"100%",left:"-6px", textAlign:"center", padding:"5px",backgroundColor:"#000000dd",display:"flex",justifyContent:"space-evenly"}}>
