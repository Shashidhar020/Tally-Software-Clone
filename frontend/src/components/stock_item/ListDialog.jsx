...
const ListDialog = ({ title, items, onClose, onSelect, variant}) => {
  ...
  return (
    <div
      style={{
        position:"absolute",
       top: isDropdown ? isListofgroups  ? "23%"  : isUnits  ? "25%"  : isListActions  ? "49%"  : isAction  ? "67%" : isSupply? "92%":  isapplicability?"40%":istaxabilitytype?"70%": "40%" : "32%",
        left: isDropdown? isListofgroups  ? "24%"  : isUnits  ? "24%"  : isListActions  ? "70%"   : isAction  ? "70%"    : isSupply  ? "66%":  isapplicability?"63%":istaxabilitytype?"68%":"40%"  : "16%",
